...
import { creditSeasonAllocationsOnChain } from "./on-chain.js";
import { Connection, PublicKey } from "@solana/web3.js";
import pg from "pg";
import { Redis } from "ioredis";
import { REDIS_KEYS, allocateClanSeasonPool, allocatePoolByXp, } from "@pump/xp";
import { PDA_SEEDS, PROGRAM_IDS, resolveSolanaRpcUrl } from "@pump/solana-sdk";
function pdaBalanceLamports(conn, seed, programId) {
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from(seed)], programId);
    return conn.getBalance(pda, "confirmed").then(BigInt);
}
async function loadClanContext(pool, clanIds) {
    const leaderByClan = new Map();
    const membersByClan = new Map();
    if (clanIds.length === 0) {
        return { leaderByClan, membersByClan };
    }
    const leaders = await pool.query(`SELECT id::text, leader_address FROM clans WHERE id = ANY($1::uuid[])`, [clanIds]);
...
async function enrichMemberXpFromArchive(redis, seasonId, membersByClan) {
...
async function main() {
    ...
    const { leaderByClan, membersByClan } = await loadClanContext(pool, topClans.map((c) => c.id));
