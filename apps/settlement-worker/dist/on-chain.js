...
import { PDA_SEEDS, PROGRAM_IDS, SEASON_POOL_KIND, encodeCreditSeasonRewardIx, resolveSolanaRpcUrl, } from "@pump/solana-sdk";
function pda(programId, seed) {
    return PublicKey.findProgramAddressSync([Buffer.from(seed)], programId)[0];
}
function pdaSeasonReward(programId, seasonId, wallet) {
    const season = Buffer.alloc(4);
    season.writeUInt32LE(seasonId, 0);
    return PublicKey.findProgramAddressSync([Buffer.from(PDA_SEEDS.seasonRewards), season, wallet.toBuffer()], programId)[0];
}
...
function buildCreditIx(input) {
    const seasonReward = pdaSeasonReward(input.programId, input.seasonId, input.beneficiary);
...
export async function creditSeasonAllocationsOnChain(input) {
    ...
    for (let i = 0; i < positive.length; i += chunkSize) {
        ...
        for (const row of chunk) {
            ...
            tx.add(buildCreditIx({
                programId,
                authority: authority.publicKey,
                global,
                sourcePool: row.poolKind === SEASON_POOL_KIND.clan ? clanPool : seasonPool,
                liquidity,
                seasonId: input.seasonId,
                beneficiary,
                amount: row.lamports,
