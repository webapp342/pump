...
export function allocatePoolByXp(
  entries: RankedEntry[],
  totalLamports: bigint
): LamportAllocation[] {
  const eligible = entries.filter((e) => e.xp > 0);
  if (eligible.length === 0 || totalLamports <= 0n) return [];
...
export function allocateClanSeasonPool(input: {
  ...
  const out: ClanWalletAllocation[] = [];
  ...
  for (const clanShare of clanShares) {
    ...
    if (!leader || clanShare.lamports <= 0n) continue;
    ...
    const memberRows = members.filter(
      (m) => m.wallet !== leader && m.xp > 0
    );
    const memberAlloc = allocatePoolByXp(
      memberRows.map((m, i) => ({
        id: m.wallet,
        xp: m.xp,
        rank: i + 1,
